...
class Automata:
    ...
    def tokenCreator(self,text,i,token=None):
        ...
        #Formar nuevo entero (not token.inFormation an self.is_digit(char))
        #Formar nuevo variable (not token.inFormation...)

        elif(
            token.inFormation
        ):
            if(
                self.is_quote(token.atFirst()) and not self.is_quote(char)
            ):
                token.add(char)
            else:
                token.add(char)
